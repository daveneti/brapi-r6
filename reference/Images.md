# Images Class

The Images class handles calling the BraAPI server and is a wrapper
class around httr2 functionality for the Image entity

## Details

The Images class handles calling the BraAPI server and is a wrapper
class around httr2 functionality for the Image entity

## See also

Other generated: [`AlleleMatrix`](AlleleMatrix.md),
[`BrAPIClient`](BrAPIClient.md),
[`BreedingMethods`](BreedingMethods.md), [`CallSets`](CallSets.md),
[`Calls`](Calls.md), [`Crosses`](Crosses.md),
[`CrossingProjects`](CrossingProjects.md), [`Events`](Events.md),
[`GenomeMaps`](GenomeMaps.md), [`Germplasm`](Germplasm.md),
[`GermplasmAttributeValues`](GermplasmAttributeValues.md),
[`GermplasmAttributes`](GermplasmAttributes.md), [`Lists`](Lists.md),
[`Locations`](Locations.md), [`MarkerPositions`](MarkerPositions.md),
[`Methods`](Methods.md), [`ObservationUnits`](ObservationUnits.md),
[`ObservationVariables`](ObservationVariables.md),
[`Observations`](Observations.md), [`Ontologies`](Ontologies.md),
[`PedigreeNodes`](PedigreeNodes.md), [`People`](People.md),
[`PlannedCrosses`](PlannedCrosses.md), [`Plates`](Plates.md),
[`Programs`](Programs.md), [`ReferenceSets`](ReferenceSets.md),
[`References`](References.md), [`Samples`](Samples.md),
[`Scales`](Scales.md), [`Seasons`](Seasons.md),
[`SeedLots`](SeedLots.md), [`Studies`](Studies.md),
[`Traits`](Traits.md), [`Trials`](Trials.md),
[`VariantSets`](VariantSets.md), [`Variants`](Variants.md)

## Methods

### Public methods

- [`Images$new()`](#method-Images-new)

- [`Images$get()`](#method-Images-get)

- [`Images$getAll()`](#method-Images-getAll)

- [`Images$search()`](#method-Images-search)

- [`Images$searchResult()`](#method-Images-searchResult)

- [`Images$create()`](#method-Images-create)

- [`Images$update()`](#method-Images-update)

- [`Images$clone()`](#method-Images-clone)

------------------------------------------------------------------------

### Method `new()`

Creates a new instance of this
[R6](https://r6.r-lib.org/reference/R6Class.html) class. It is not
recommended that this object is created separately from the BrAPIClient

#### Usage

    Images$new(client)

#### Arguments

- `client`:

  THe BrAPIClient to be used for server communication

------------------------------------------------------------------------

### Method [`get()`](https://rdrr.io/r/base/get.html)

Gets a Image object by DbId from the BrAPI server

#### Usage

    Images$get(id)

#### Arguments

- `id`:

  The DbIid of the Image to be returned

#### Returns

returns a Image object by ID.

------------------------------------------------------------------------

### Method `getAll()`

Gets a list Image objects from the BrAPI server Note the filtering
arguments are all optional and can be combined to filter the results The
value of the filters an be a single value or a vector of values

#### Usage

    Images$getAll(
      commonCropNames = NULL,
      programDbIds = NULL,
      programNames = NULL,
      descriptiveOntologyTerms = NULL,
      imageFileNames = NULL,
      imageFileSizeMax = NULL,
      imageFileSizeMin = NULL,
      imageHeightMax = NULL,
      imageHeightMin = NULL,
      imageLocation = NULL,
      imageNames = NULL,
      imageTimeStampRangeEnd = NULL,
      imageTimeStampRangeStart = NULL,
      imageWidthMax = NULL,
      imageWidthMin = NULL,
      mimeTypes = NULL,
      observationDbIds = NULL,
      imageDbIds = NULL,
      observationUnitDbIds = NULL,
      page = 0,
      pageSize = 1000
    )

#### Arguments

- `commonCropNames`:

  The BrAPI Common Crop Name is the simple, generalized, widely accepted
  name of the organism being researched.

- `programDbIds`:

  A BrAPI Program represents the high level organization or group who is
  responsible for conducting trials and studies.

- `programNames`:

  Use this parameter to only return results associated with the given
  program names.

- `descriptiveOntologyTerms`:

  A list of terms to formally describe the image to search for.

- `imageFileNames`:

  Image file names to search for.

- `imageFileSizeMax`:

  A maximum image file size to search for.

- `imageFileSizeMin`:

  A minimum image file size to search for.

- `imageHeightMax`:

  A maximum image height to search for.

- `imageHeightMin`:

  A minimum image height to search for.

- `imageLocation`:

- `imageNames`:

  Human readable names to search for.

- `imageTimeStampRangeEnd`:

  The latest timestamp to search for.

- `imageTimeStampRangeStart`:

  The earliest timestamp to search for.

- `imageWidthMax`:

  A maximum image width to search for.

- `imageWidthMin`:

  A minimum image width to search for.

- `mimeTypes`:

  A set of image file types to search for.

- `observationDbIds`:

  A list of observation Ids this image is associated with to search for

- `imageDbIds`:

  A list of image Ids to search for

- `observationUnitDbIds`:

  A set of observation unit identifiers to search for.

- `page`:

  The page number of results to return, starting from 0

- `pageSize`:

  The maximum number of results to return per page

#### Returns

returns a paged and filtered list of Image objects.

------------------------------------------------------------------------

### Method [`search()`](https://rdrr.io/r/base/search.html)

Searches for Image objects on the BrAPI server. The server may return
the paged results or a search result ID for later retrieval

#### Usage

    Images$search(
      commonCropNames = NULL,
      programDbIds = NULL,
      programNames = NULL,
      descriptiveOntologyTerms = NULL,
      imageFileNames = NULL,
      imageFileSizeMax = NULL,
      imageFileSizeMin = NULL,
      imageHeightMax = NULL,
      imageHeightMin = NULL,
      imageLocation = NULL,
      imageNames = NULL,
      imageTimeStampRangeEnd = NULL,
      imageTimeStampRangeStart = NULL,
      imageWidthMax = NULL,
      imageWidthMin = NULL,
      mimeTypes = NULL,
      observationDbIds = NULL,
      imageDbIds = NULL,
      observationUnitDbIds = NULL,
      page = 0,
      pageSize = 1000
    )

#### Arguments

- `commonCropNames`:

  The BrAPI Common Crop Name is the simple, generalized, widely accepted
  name of the organism being researched.

- `programDbIds`:

  A BrAPI Program represents the high level organization or group who is
  responsible for conducting trials and studies.

- `programNames`:

  Use this parameter to only return results associated with the given
  program names.

- `descriptiveOntologyTerms`:

  A list of terms to formally describe the image to search for.

- `imageFileNames`:

  Image file names to search for.

- `imageFileSizeMax`:

  A maximum image file size to search for.

- `imageFileSizeMin`:

  A minimum image file size to search for.

- `imageHeightMax`:

  A maximum image height to search for.

- `imageHeightMin`:

  A minimum image height to search for.

- `imageLocation`:

- `imageNames`:

  Human readable names to search for.

- `imageTimeStampRangeEnd`:

  The latest timestamp to search for.

- `imageTimeStampRangeStart`:

  The earliest timestamp to search for.

- `imageWidthMax`:

  A maximum image width to search for.

- `imageWidthMin`:

  A minimum image width to search for.

- `mimeTypes`:

  A set of image file types to search for.

- `observationDbIds`:

  A list of observation Ids this image is associated with to search for

- `imageDbIds`:

  A list of image Ids to search for

- `observationUnitDbIds`:

  A set of observation unit identifiers to search for. using the
  searchResult function

- `page`:

  The page number of results to return, starting from 0

- `pageSize`:

  The maximum number of results to return per page

#### Returns

returns the search result or an ID to the results.

Get the result of a search for Image objects on the BrAPI server. If the
server returned search result ID for later retrieval, use this function
to get the results.

------------------------------------------------------------------------

### Method `searchResult()`

#### Usage

    Images$searchResult(searchResultId, page = 0, pageSize = 1000)

#### Arguments

- `searchResultId`:

  The search result ID returned from a previous search call

- `page`:

  The page number of results to return, starting from 0

- `pageSize`:

  The maximum number of results to return per page

#### Returns

returns the search result or by ID to the results.

------------------------------------------------------------------------

### Method `create()`

Creates a Image object on the BrAPI server.

#### Usage

    Images$create(newValue)

#### Arguments

- `newValue`:

  The new Image object to be created

------------------------------------------------------------------------

### Method [`update()`](https://rdrr.io/r/stats/update.html)

Creates a Image object on the BrAPI server.

#### Usage

    Images$update(value)

#### Arguments

- `value`:

  The updated Image object to be sent to the server

------------------------------------------------------------------------

### Method `clone()`

The objects of this class are cloneable with this method.

#### Usage

    Images$clone(deep = FALSE)

#### Arguments

- `deep`:

  Whether to make a deep clone.
