...
The References class handles calling the BraAPI server and is a wrapper
class around httr2 functionality for the Reference entity
...
- [`References$new()`](#method-References-new)

- [`References$get()`](#method-References-get)

- [`References$getAll()`](#method-References-getAll)

- [`References$search()`](#method-References-search)

- [`References$searchResult()`](#method-References-searchResult)

- [`References$clone()`](#method-References-clone)
...
- `id`:
  ...
  The DbIid of the Reference to be returned
...
Gets a list Reference objects from the BrAPI server Note the filtering
...
- `accessions`:
  ...
  If specified, return the references for which the `accession` matches
  this string (case-sensitive, exact match).

- `md5checksums`:

  If specified, return the references for which the `md5checksum`
  matches this string (case-sensitive, exact match).

- `referenceDbIds`:

  A list of IDs which uniquely identify `References` within the given
  database server

- `referenceSetDbIds`:

  A list of IDs which uniquely identify `ReferenceSets` within the given
  database server
...
returns a paged and filtered list of Reference objects.

------------------------------------------------------------------------

### Method [`search()`](https://rdrr.io/r/base/search.html)

Searches for Reference objects on the BrAPI server. The server may
...
- `maxLength`:
  ...
  The minimum length of this `References` sequence. using the
...
Get the result of a search for Reference objects on the BrAPI server. If
...
#### Usage
    ...
    References$searchResult(searchResultId, page = 0, pageSize = 1000)
